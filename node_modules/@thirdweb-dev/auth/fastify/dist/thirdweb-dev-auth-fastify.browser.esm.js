...
import { z } from 'zod';
import { l as LoginPayloadOutputSchema, o as THIRDWEB_AUTH_TOKEN_COOKIE_PREFIX, n as THIRDWEB_AUTH_ACTIVE_ACCOUNT_COOKIE, q as THIRDWEB_AUTH_DEFAULT_REFRESH_INTERVAL_IN_SECONDS, t as THIRDWEB_AUTH_DEFAULT_TOKEN_DURATION_IN_SECONDS, T as ThirdwebAuth$1 } from '../../dist/auth-ffe5a232.browser.esm.js';
...
const loginHandler = (fastify, ctx) => {
  fastify.route({
    ...
    handler: async (req, res) => {
      ...
      res.setCookie(`${THIRDWEB_AUTH_TOKEN_COOKIE_PREFIX}_${req.body.payload.payload.address}`, token, {
      ...
      res.setCookie(THIRDWEB_AUTH_ACTIVE_ACCOUNT_COOKIE, req.body.payload.payload.address, {
        domain: ctx.cookieOptions?.domain,
        path: ctx.cookieOptions?.path || "/",
        sameSite: ctx.cookieOptions?.sameSite || "none",
        expires: new Date(exp * 1000),
      ...
      if (user) {
        ...
        if (token) {
          ...
          if (new Date() > refreshDate) {
            ...
            res.setCookie(`${THIRDWEB_AUTH_TOKEN_COOKIE_PREFIX}_${user.address}`, refreshedToken, {
              domain: ctx.cookieOptions?.domain,
              path: ctx.cookieOptions?.path || "/",
              sameSite: ctx.cookieOptions?.sameSite || "none",
...
const switchAccountHandler = (fastify, ctx) => {
  fastify.route({
    ...
    handler: async (req, res) => {
      ...
      const cookie = getCookie(req, `${THIRDWEB_AUTH_TOKEN_COOKIE_PREFIX}_${req.body.address}`);
      if (cookie) {
...
export { ActiveBodySchema, ErrorResponseSchema, LoginPayloadBodySchema, PayloadBodySchema, ThirdwebAuth, getToken };
