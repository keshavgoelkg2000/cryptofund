...
var marketplaceAuction = require('./marketplace-auction-5b471186.cjs.dev.js');
require('zod');
 ...
 */
class Marketplace {
  static contractRoles = smartContract.MARKETPLACE_CONTRACT_ROLES;
  ...
  constructor(network, address, storage) {
    ...
    let contractWrapper = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : new smartContract.ContractWrapper(network, address, abi, options, storage);
    this._chainId = chainId;
    this.abi = smartContract.AbiSchema.parse(abi || []);
    this.contractWrapper = contractWrapper;
    this.storage = storage;
    this.metadata = new smartContract.ContractMetadata(this.contractWrapper, smartContract.MarketplaceContractSchema, this.storage);
    this.app = new smartContract.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new smartContract.ContractRoles(this.contractWrapper, Marketplace.contractRoles);
    ...
    this.platformFees = new smartContract.ContractPlatformFee(this.contractWrapper);
  ...
  async isRestrictedToListerRoleOnly() {
    const anyoneCanList = await this.contractWrapper.read("hasRole", [smartContract.getRoleHash("lister"), ethers.constants.AddressZero]);
  ...
  async getOffers(listingId) {
    ...
    return await Promise.all(listingEvents.map(e => {
      return smartContract.mapOffer(this.contractWrapper.getProvider(), ethers.BigNumber.from(listingId), {
   ...
   * ```
   */
  makeOffer = /* @__PURE__ */smartContract.buildTransactionFunction(async (listingId, pricePerToken, quantity) => {
    const listing = await this.contractWrapper.read("listings", [listingId]);
    if (listing.listingId.toString() !== listingId.toString()) {
      throw new smartContract.ListingNotFoundError(this.getAddress(), listingId.toString());
    }
      ...
      case marketplaceAuction.ListingType.Direct:
        {
          ...
          return await this.direct.makeOffer.prepare(listingId, quantity, smartContract.isNativeToken(listing.currency) ? smartContract.NATIVE_TOKENS[chainId].wrapped.address : listing.currency, pricePerToken);
        }
      case marketplaceAuction.ListingType.Auction:
        {
   ...
   */
  setBidBufferBps = /* @__PURE__ */smartContract.buildTransactionFunction(async bufferBps => {
   ...
   */
  setTimeBufferInSeconds = /* @__PURE__ */smartContract.buildTransactionFunction(async bufferInSeconds => {
    await this.roles.verify(["admin"], await this.contractWrapper.getSignerAddress());
   ...
   */
  allowListingFromSpecificAssetOnly = /* @__PURE__ */smartContract.buildTransactionFunction(async contractAddress => {
    const encoded = [];
    const members = await this.roles.get("asset");
    if (members.includes(ethers.constants.AddressZero)) {
      encoded.push(this.encoder.encode("revokeRole", [smartContract.getRoleHash("asset"), ethers.constants.AddressZero]));
    }
    encoded.push(this.encoder.encode("grantRole", [smartContract.getRoleHash("asset"), contractAddress]));
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "multicall",
      args: [encoded]
   ...
   */
  allowListingFromAnyAsset = /* @__PURE__ */smartContract.buildTransactionFunction(async () => {
    const encoded = [];
    const members = await this.roles.get("asset");
    for (const addr in members) {
      encoded.push(this.encoder.encode("revokeRole", [smartContract.getRoleHash("asset"), addr]));
    }
    encoded.push(this.encoder.encode("grantRole", [smartContract.getRoleHash("asset"), ethers.constants.AddressZero]));
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "multicall",
      args: [encoded]
  ...
  async getAllListingsNoFilter(filterInvalidListings) {
    const listings = await Promise.all(Array.from(Array((await this.contractWrapper.read("totalListings", [])).toNumber()).keys()).map(async i => {
      ...
      try {
        ...
        if (err instanceof smartContract.ListingNotFoundError) {
          return undefined;
        } else {
  ...
  applyFilter(listings, filter) {
    ...
    const count = ethers.BigNumber.from(filter?.count || smartContract.DEFAULT_QUERY_ALL_COUNT).toNumber();
    if (filter) {
      if (filter.seller) {
