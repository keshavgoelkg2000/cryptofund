 ...
 */
class Split {
  static contractRoles = smartContract.ADMIN_ROLE;
  ...
  constructor(network, address, storage) {
    ...
    this.metadata = new smartContract.ContractMetadata(this.contractWrapper, smartContract.SplitsContractSchema, this.storage);
    this.app = new smartContract.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new smartContract.ContractRoles(this.contractWrapper, Split.contractRoles);
  ...
  async balanceOfTokenAllRecipients(tokenAddress) {
    const [resolvedToken, recipients] = await Promise.all([smartContract.resolveAddress(tokenAddress), this.getAllRecipients()]);
    const balances = {};
    for (const recipient of recipients) {
  ...
  async balanceOf(address) {
    const [resolvedAddress, walletBalance, totalReleased] = await Promise.all([smartContract.resolveAddress(address), this.contractWrapper.getProvider().getBalance(this.getAddress()), this.contractWrapper.read("totalReleased", [])]);
  ...
  async balanceOfToken(walletAddress, tokenAddress) {
    const [resolvedToken, resolvedWallet] = await Promise.all([smartContract.resolveAddress(tokenAddress), smartContract.resolveAddress(walletAddress)]);
    ...
    return await smartContract.fetchCurrencyValue(this.contractWrapper.getProvider(), resolvedToken, value);
  }
  ...
  async getRecipientSplitPercentage(address) {
    const [resolvedAddress, totalShares, walletsShares] = await Promise.all([smartContract.resolveAddress(address), this.contractWrapper.read("totalShares", []), this.contractWrapper.read("shares", [address])]);
   ...
   */
  withdraw = /* @__PURE__ */smartContract.buildTransactionFunction(async walletAddress => {
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "release(address)",
      args: [await smartContract.resolveAddress(walletAddress)]
    });
  });
   ...
   */
  withdrawToken = /* @__PURE__ */smartContract.buildTransactionFunction(async (walletAddress, tokenAddress) => {
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "release(address,address)",
      args: await Promise.all([smartContract.resolveAddress(tokenAddress), smartContract.resolveAddress(walletAddress)])
    });
  });
   ...
   * ```
   */
  distribute = /* @__PURE__ */smartContract.buildTransactionFunction(async () => {
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "distribute()",
      args: []
   ...
   * @param tokenAddress - The address of the currency contract to distribute funds
   */
  distributeToken = /* @__PURE__ */smartContract.buildTransactionFunction(async tokenAddress => {
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "distribute(address)",
      args: [await smartContract.resolveAddress(tokenAddress)]
    });
  });
  ...
  async _pendingPayment(address, totalReceived, alreadyReleased) {
    const [resolvedAddress, totalShares] = await Promise.all([smartContract.resolveAddress(address), this.contractWrapper.read("totalShares", [])]);
