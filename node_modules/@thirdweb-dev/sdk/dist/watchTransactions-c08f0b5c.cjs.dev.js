...
async function getContractFromAbi(params) {
  const [signer, provider] = smartContract.getSignerAndProvider(params.network, params.sdkOptions);
  const [resolvedAddress, {
    chainId
  }] = await Promise.all([smartContract.resolveAddress(params.address), provider.getNetwork()]);
  ...
  const contract = new smartContract.SmartContract(signer || provider, resolvedAddress, await smartContract.getCompositeABI(resolvedAddress, smartContract.AbiSchema.parse(parsedAbi), provider, params.sdkOptions, getCachedStorage(params.storage)), getCachedStorage(params.storage), params.sdkOptions, chainId);
...
async function getContract(params) {
  ...
  if (!params.contractTypeOrAbi || params.contractTypeOrAbi === "custom") {
    ...
    if (contractType === "custom") {
      const publisher = new smartContract.ContractPublisher(params.network, params.sdkOptions, getCachedStorage(params.storage));
      try {
      ...
      const abi = await smartContract.PREBUILT_CONTRACTS_MAP[contractType].getAbi(resolvedAddress, provider, getCachedStorage(params.storage));
      return getContractFromAbi({
        ...params,
        address: resolvedAddress,
        abi
  ...
  } else if (typeof params.contractTypeOrAbi === "string" && params.contractTypeOrAbi in smartContract.PREBUILT_CONTRACTS_MAP) {
    const contract = await smartContract.PREBUILT_CONTRACTS_MAP[params.contractTypeOrAbi].initialize(signer || provider, resolvedAddress, getCachedStorage(params.storage), params.sdkOptions);
    cacheContract(contract, resolvedAddress, chainId);
    return contract;
  } else {
    return getContractFromAbi({
...
async function getChainId(params) {
  const [, provider] = smartContract.getSignerAndProvider(params.network, params.sdkOptions);
  let chainIdPromise;
