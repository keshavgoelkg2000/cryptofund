...
var hasERC20Allowance = require('./hasERC20Allowance-e2722a66.cjs.dev.js');
var erc1155Standard = require('./erc-1155-standard-76b42da6.cjs.dev.js');
...
var pack = require('./pack-34f95cae.cjs.dev.js');
require('zod');
...
class PackVRF {
  featureName = smartContract.FEATURE_PACK_VRF.name;
  constructor(network, address, storage, options, chainId) {
    let contractWrapper = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : new smartContract.ContractWrapper(network, address, IPackVRFAbi__default["default"], options, storage);
    this.contractWrapper = contractWrapper;
    this.storage = storage;
    this.chainId = chainId;
    this.events = new smartContract.ContractEvents(this.contractWrapper);
  }
   ...
   * @twfeature PackVRF
   */
  open = /* @__PURE__ */smartContract.buildTransactionFunction((() => {
    var _this = this;
   ...
   * @twfeature PackVRF
   */
  claimRewards = /* @__PURE__ */smartContract.buildTransactionFunction((() => {
    var _this2 = this;
  ...
  async parseRewards(rewards) {
    ...
    for (const reward of rewards) {
      switch (reward.tokenType) {
        case 0:
          {
            const tokenMetadata = await smartContract.fetchCurrencyMetadata(this.contractWrapper.getProvider(), reward.assetContract);
            erc20Rewards.push({
  ...
  async canClaimRewards(claimerAddress) {
    const address = await smartContract.resolveAddress(claimerAddress || (await this.contractWrapper.getSignerAddress()));
  ...
  getLinkContract() {
    const linkAddress = smartContract.LINK_TOKEN_ADDRESS[this.chainId];
    if (!linkAddress) {
      throw new Error(`No LINK token address found for chainId ${this.chainId}`);
    }
    const contract = new smartContract.ContractWrapper(this.contractWrapper.getSignerOrProvider(), linkAddress, ERC20Abi__default["default"], this.contractWrapper.options, this.storage);
    return new smartContract.Erc20(contract, this.storage, this.chainId);
  }
}
...
class Pack extends erc1155Standard.StandardErc1155 {
  static contractRoles = smartContract.PACK_CONTRACT_ROLES;
   ...
   */
  get vrf() {
    return smartContract.assertEnabled(this._vrf, smartContract.FEATURE_PACK_VRF);
  }
  constructor(network, address, storage) {
    let options = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};
    let abi = arguments.length > 4 ? arguments[4] : undefined;
    let chainId = arguments.length > 5 ? arguments[5] : undefined;
    let contractWrapper = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : new smartContract.ContractWrapper(network, address, abi, options.gasless && "openzeppelin" in options.gasless ? {
      ...options,
      gasless: {
        ...options.gasless,
        openzeppelin: {
    ...
    this.abi = smartContract.AbiSchema.parse(abi || []);
    this.metadata = new smartContract.ContractMetadata(this.contractWrapper, smartContract.PackContractSchema, this.storage);
    this.app = new smartContract.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new smartContract.ContractRoles(this.contractWrapper, Pack.contractRoles);
    this.royalties = new smartContract.ContractRoyalty(this.contractWrapper, this.metadata);
    this.encoder = new smartContract.ContractEncoder(this.contractWrapper);
    this.estimator = new smartContract.GasCostEstimator(this.contractWrapper);
    this.events = new smartContract.ContractEvents(this.contractWrapper);
    this.interceptor = new smartContract.ContractInterceptor(this.contractWrapper);
    this.owner = new smartContract.ContractOwner(this.contractWrapper);
  ...
  async isTransferRestricted() {
    const anyoneCanTransfer = await this.contractWrapper.read("hasRole", [smartContract.getRoleHash("transfer"), ethers.constants.AddressZero]);
   ...
   * ```
   */
  create = /* @__PURE__ */smartContract.buildTransactionFunction(async metadataWithRewards => {
   ...
   * ```
   */
  addPackContents = /* @__PURE__ */smartContract.buildTransactionFunction(async (packId, packContents) => {
   ...
   * ```
   */
  createTo = /* @__PURE__ */smartContract.buildTransactionFunction(async (to, metadataWithRewards) => {
    const [uri, parsedMetadata, toAddress] = await Promise.all([smartContract.uploadOrExtractURI(metadataWithRewards.packMetadata, this.storage), pack.PackMetadataInputSchema.parseAsync(metadataWithRewards), smartContract.resolveAddress(to)]);
    const {
      erc20Rewards,
      erc721Rewards,
      erc1155Rewards
        ...
        parse: async receipt => {
          ...
          for (const reward of rewards) {
            switch (reward.tokenType) {
              case 0:
                {
                  const tokenMetadata = await smartContract.fetchCurrencyMetadata(_this.contractWrapper.getProvider(), reward.assetContract);
                  erc20Rewards.push({
                    contractAddress: reward.assetContract,
                    quantityPerReward: ethers.utils.formatUnits(reward.totalAmount, tokenMetadata.decimals).toString()
                  });
  ...
  async toPackContentArgs(metadataWithRewards) {
    ...
    for (const erc20 of erc20Rewards) {
      const normalizedQuantity = await smartContract.normalizePriceValue(provider, erc20.quantityPerReward, erc20.contractAddress);
    ...
    for (const erc721 of erc721Rewards) {
      const isApproved = await smartContract.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), erc721.contractAddress, erc721.tokenId, owner);
      if (!isApproved) {
    ...
    for (const erc1155 of erc1155Rewards) {
      const isApproved = await smartContract.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), erc1155.contractAddress, erc1155.tokenId, owner);
      if (!isApproved) {
  ...
  detectVrf() {
    if (smartContract.detectContractFeature(this.contractWrapper, "PackVRF")) {
