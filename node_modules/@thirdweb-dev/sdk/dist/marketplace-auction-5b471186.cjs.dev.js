...
class MarketplaceDirect {
  ...
  async getListing(listingId) {
    ...
    if (listing.assetContract === ethers.constants.AddressZero) {
      throw new smartContract.ListingNotFoundError(this.getAddress(), listingId.toString());
    }
    if (listing.listingType !== ListingType.Direct) {
      throw new smartContract.WrongListingTypeError(this.getAddress(), listingId.toString(), "Auction", "Direct");
    }
  ...
  async getActiveOffer(listingId, address) {
    ...
    const offers = await this.contractWrapper.read("offers", [listingId, await smartContract.resolveAddress(address)]);
    if (offers.offeror === ethers.constants.AddressZero) {
      return undefined;
    }
    return await smartContract.mapOffer(this.contractWrapper.getProvider(), ethers.BigNumber.from(listingId), offers);
  }
   ...
   * ```
   */
  createListing = /* @__PURE__ */smartContract.buildTransactionFunction(async listing => {
    smartContract.validateNewListingParam(listing);
    const resolvedAssetAddress = await smartContract.resolveAddress(listing.assetContractAddress);
    const resolvedCurrencyAddress = await smartContract.resolveAddress(listing.currencyContractAddress);
    await smartContract.handleTokenApproval(this.contractWrapper, this.getAddress(), resolvedAssetAddress, listing.tokenId, await this.contractWrapper.getSignerAddress());
    const normalizedPricePerToken = await smartContract.normalizePriceValue(this.contractWrapper.getProvider(), listing.buyoutPricePerToken, resolvedCurrencyAddress);
    ...
    return smartContract.Transaction.fromContractWrapper({
      ...
      args: [{
        ...
        currencyToAccept: smartContract.cleanCurrencyAddress(resolvedCurrencyAddress),
   ...
   * ```
   */
  createListingsBatch = /* @__PURE__ */smartContract.buildTransactionFunction(async listings => {
   ...
   * ```
   */
  makeOffer = /* @__PURE__ */smartContract.buildTransactionFunction(async (listingId, quantityDesired, currencyContractAddress, pricePerToken, expirationDate) => {
    if (smartContract.isNativeToken(currencyContractAddress)) {
      throw new Error("You must use the wrapped native token address when making an offer with a native token");
    }
    const normalizedPrice = await smartContract.normalizePriceValue(this.contractWrapper.getProvider(), pricePerToken, currencyContractAddress);
    try {
    ...
    await smartContract.setErc20Allowance(this.contractWrapper, value, currencyContractAddress, overrides);
   ...
   * ```
   */
  acceptOffer = /* @__PURE__ */smartContract.buildTransactionFunction(async (listingId, addressOfOfferor) => {
    /**
     * TODO:
     * - Provide better error handling if offer is too low.
     */
    await this.validateListing(ethers.BigNumber.from(listingId));
    const resolvedAddress = await smartContract.resolveAddress(addressOfOfferor);
   ...
   */
  buyoutListing = /* @__PURE__ */smartContract.buildTransactionFunction(async (listingId, quantityDesired, receiver) => {
    ...
    const overrides = (await this.contractWrapper.getCallOverrides()) || {};
    await smartContract.setErc20Allowance(this.contractWrapper, value, listing.currencyContractAddress, overrides);
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "buy",
   ...
   */
  updateListing = /* @__PURE__ */smartContract.buildTransactionFunction(async listing => {
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "updateListing",
      args: [listing.id, listing.quantity, listing.buyoutPrice,
      // reserve price, doesn't matter for direct listing
      listing.buyoutPrice, await smartContract.resolveAddress(listing.currencyContractAddress), listing.startTimeInSeconds, listing.secondsUntilEnd]
    });
  });
   ...
   * ```
   */
  cancelListing = /* @__PURE__ */smartContract.buildTransactionFunction(async listingId => {
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
  ...
  async mapListing(listing) {
    return {
      ...
      buyoutCurrencyValuePerToken: await smartContract.fetchCurrencyValue(this.contractWrapper.getProvider(), listing.currency, listing.buyoutPricePerToken),
      id: listing.listingId.toString(),
      tokenId: listing.tokenId,
      quantity: listing.quantity,
      startTimeInSeconds: listing.startTime,
      asset: await smartContract.fetchTokenMetadataForContract(listing.assetContract, this.contractWrapper.getProvider(), listing.tokenId, this.storage),
  ...
  async isStillValidListing(listing, quantity) {
    const approved = await smartContract.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), listing.assetContractAddress, listing.tokenId, listing.sellerAddress);
    if (!approved) {
      return {
        valid: false,
    ...
    const isERC721 = await erc165.supportsInterface(smartContract.InterfaceId_IERC721);
    const isERC1155 = await erc165.supportsInterface(smartContract.InterfaceId_IERC1155);
    if (isERC721) {
...
class MarketplaceAuction {
  constructor(contractWrapper, storage) {
    this.contractWrapper = contractWrapper;
    this.storage = storage;
    this.encoder = new smartContract.ContractEncoder(contractWrapper);
  }
  getAddress() {
    return this.contractWrapper.address;
  }
  ...
  async getListing(listingId) {
    ...
    if (listing.listingType !== ListingType.Auction) {
      throw new smartContract.WrongListingTypeError(this.getAddress(), listingId.toString(), "Direct", "Auction");
    }
    return await this.mapListing(listing);
  }
  ...
  async getWinner(listingId) {
    ...
    const contractEvents = new smartContract.ContractEvents(this.contractWrapper);
  ...
  createListing = /* @__PURE__ */smartContract.buildTransactionFunction(async listing => {
    ...
    await smartContract.handleTokenApproval(this.contractWrapper, this.getAddress(), resolvedAssetAddress, listing.tokenId, await this.contractWrapper.getSignerAddress());
    const normalizedPricePerToken = await smartContract.normalizePriceValue(this.contractWrapper.getProvider(), listing.buyoutPricePerToken, resolvedCurrencyAddress);
    const normalizedReservePrice = await smartContract.normalizePriceValue(this.contractWrapper.getProvider(), listing.reservePricePerToken, resolvedCurrencyAddress);
    let listingStartTime = Math.floor(listing.startTimestamp.getTime() / 1000);
    const block = await this.contractWrapper.getProvider().getBlock("latest");
    const blockTime = block.timestamp;
    if (listingStartTime < blockTime) {
   ...
   * ```
   */
  buyoutListing = /* @__PURE__ */smartContract.buildTransactionFunction(async listingId => {
    const listing = await this.validateListing(ethers.BigNumber.from(listingId));
    const currencyMetadata = await smartContract.fetchCurrencyMetadata(this.contractWrapper.getProvider(), listing.currencyContractAddress);
   ...
   * ```
   */
  makeBid = /* @__PURE__ */smartContract.buildTransactionFunction(async (listingId, pricePerToken) => {
    const listing = await this.validateListing(ethers.BigNumber.from(listingId));
    const normalizedPrice = await smartContract.normalizePriceValue(this.contractWrapper.getProvider(), pricePerToken, listing.currencyContractAddress);
    ...
    if (winningBid) {
      const isWinner = smartContract.isWinningBid(winningBid.pricePerToken, normalizedPrice, bidBuffer);
  ...
  cancelListing = /* @__PURE__ */smartContract.buildTransactionFunction(async listingId => {
    ...
    if (now.gt(startTime) && offers.offeror !== ethers.constants.AddressZero) {
      throw new smartContract.AuctionAlreadyStartedError(listingId.toString());
    }
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "closeAuction",
   ...
   */
  closeListing = /* @__PURE__ */smartContract.buildTransactionFunction(async (listingId, closeFor) => {
    if (!closeFor) {
    ...
    try {
      ...
      if (err.message.includes("cannot close auction before it has ended")) {
        throw new smartContract.AuctionHasNotEndedError(listingId.toString(), listing.endTimeInEpochSeconds.toString());
      } else {
        throw err;
      }
    }
   ...
   * @param listingId - the auction  listing ud to close
   */
  executeSale = /* @__PURE__ */smartContract.buildTransactionFunction(async listingId => {
    const listing = await this.validateListing(ethers.BigNumber.from(listingId));
    try {
      const winningBid = await this.getWinningBid(listingId);
  ...
  async getMinimumNextBid(listingId) {
    ...
    return smartContract.fetchCurrencyValue(this.contractWrapper.getProvider(), listing.currencyContractAddress, minimumNextBid);
  }
  ...
  async mapListing(listing) {
    return {
      ...
      asset: await smartContract.fetchTokenMetadataForContract(listing.assetContract, this.contractWrapper.getProvider(), listing.tokenId, this.storage),
      reservePriceCurrencyValuePerToken: await smartContract.fetchCurrencyValue(this.contractWrapper.getProvider(), listing.currency, listing.reservePricePerToken),
