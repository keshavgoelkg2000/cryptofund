...
var smartContract = require('./smart-contract-281ba730.cjs.dev.js');
...
class UserWallet {
  ...
  constructor(network, options, storage) {
    this.connection = new smartContract.RPCConnectionHandler(network, options);
  ...
  async transfer(to, amount) {
    let currencyAddress = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : smartContract.NATIVE_TOKEN_ADDRESS;
    const [resolvedTo, resolvedCurrency, amountInWei] = await Promise.all([smartContract.resolveAddress(to), smartContract.resolveAddress(currencyAddress), smartContract.normalizePriceValue(this.connection.getProvider(), amount, currencyAddress)]);
    const signer = this.requireWallet();
    if (smartContract.isNativeToken(resolvedCurrency)) {
   ...
   */
  async balance() {
    let currencyAddress = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : smartContract.NATIVE_TOKEN_ADDRESS;
    this.requireWallet();
    const resolvedCurrency = await smartContract.resolveAddress(currencyAddress);
    ...
    return await smartContract.fetchCurrencyValue(provider, resolvedCurrency, balance);
  }
  ...
  async signTypedData(domain, types, message) {
    return await smartContract.signTypedDataInternal(this.requireWallet(), domain, types, message);
  }
  ...
  async sendRawTransaction(transactionRequest) {
    ...
    if (!hasGasData) {
      ...
      const defaultGas = await smartContract.getDefaultGasOverrides(this.connection.getProvider());
  ...
  async requestFunds(amount) {
    ...
    if (chainId === smartContract.ChainId.Localhost || chainId === smartContract.ChainId.Hardhat) {
      const localWallet = new UserWallet(new ethers.Wallet(LOCAL_NODE_PKEY, smartContract.getChainProvider(chainId, this.options)), this.options, this.storage);
  ...
  createErc20(currencyAddress) {
    return new smartContract.ContractWrapper(this.connection.getSignerOrProvider(), currencyAddress, ERC20Abi__default["default"], this.options, this.storage);
  }
}
...
function getAllDetectedFeatureNames(abi) {
  const features = [];
  smartContract.extractFeatures(smartContract.detectFeatures(abi), features);
...
class MultichainRegistry {
  constructor(network, storage) {
    ...
    this.storage = storage;
    this.registryLogic = new smartContract.ContractWrapper(network, smartContract.getMultichainRegistryAddress(), TWRegistryABI__default["default"], options, storage);
    this.registryRouter = new smartContract.ContractWrapper(network, smartContract.getMultichainRegistryAddress(), TWRegistryRouterABI__default["default"], options, storage);
  }
  ...
  async getContractMetadataURI(chainId, address) {
    return await this.registryLogic.read("getMetadataUri", [chainId, await smartContract.resolveAddress(address)]);
  }
  ...
  async getContractAddresses(walletAddress) {
    return (await this.registryLogic.read("getAll", [await smartContract.resolveAddress(walletAddress)])).filter(result => ethers.utils.isAddress(result.deploymentAddress) && result.deploymentAddress.toLowerCase() !== ethers.constants.AddressZero).map(result => ({
      address: result.deploymentAddress,
      chainId: result.chainId.toNumber()
    }));
  }
  addContract = /* @__PURE__ */smartContract.buildTransactionFunction(async contract => {
    return await this.addContracts.prepare([contract]);
  });
  addContracts = /* @__PURE__ */smartContract.buildTransactionFunction(async contracts => {
    const deployerAddress = await this.registryRouter.getSignerAddress();
    const encoded = [];
    const contractEncoder = new smartContract.ContractEncoder(this.registryLogic);
    ...
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this.registryRouter,
      method: "multicall",
      args: [encoded]
    });
  });
  removeContract = /* @__PURE__ */smartContract.buildTransactionFunction(async contract => {
    return await this.removeContracts.prepare([contract]);
  });
  removeContracts = /* @__PURE__ */smartContract.buildTransactionFunction(async contracts => {
    const deployerAddress = await this.registryRouter.getSignerAddress();
    const contractEncoder = new smartContract.ContractEncoder(this.registryLogic);
    const encoded = await Promise.all(contracts.map(async contract => contractEncoder.encode("remove", [deployerAddress, await smartContract.resolveAddress(contract.address), contract.chainId])));
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this.registryRouter,
      method: "multicall",
      args: [encoded]
...
async function getEncodedConstructorParamsForThirdwebContract(contractName, chainId, storage) {
  ...
  const provider = smartContract.getChainProvider(chainId, {
    clientId,
    secretKey
  });
  const publishedContract = await smartContract.fetchPublishedContractFromPolygon(smartContract.THIRDWEB_DEPLOYER, contractName, contractVersion, storage, clientId, secretKey);
  const publishUri = publishedContract.metadataUri;
  const metadata = await smartContract.fetchAndCacheDeployMetadata(publishUri, storage);
  const create2Factory = await smartContract.getCreate2FactoryAddress(provider);
  invariant__default["default"](create2Factory, "Thirdweb stack not found");
  const {
    extendedMetadata
  } = await smartContract.fetchAndCacheDeployMetadata(publishUri, storage);
  let encodedArgs;
  ...
  if (extendedMetadata?.routerType === "plugin" || extendedMetadata?.routerType === "dynamic") {
    const deploymentInfo = await smartContract.getDeploymentInfo(publishUri, storage, provider, create2Factory, clientId, secretKey);
    encodedArgs = deploymentInfo.find(contract => contract.type === "implementation")?.encodedArgs;
  } else {
    encodedArgs = await smartContract.encodeConstructorParamsForImplementation(metadata.compilerMetadata, provider, storage, create2Factory, constructorParamMap, clientId, secretKey);
  }
  return encodedArgs;
}
...
async function predictThirdwebContractAddress(contractName, chainId, storage) {
  ...
  const implementation = await smartContract.computeDeploymentInfo("implementation", provider, storage, create2Factory, {
...
async function getThirdwebContractAddress(contractName, chainId, storage) {
  ...
  const isDeployed = await smartContract.isContractDeployed(contractAddress, provider);
...
async function verify(contractAddress, chainId, explorerAPIUrl, explorerAPIKey, storage, encodedConstructorArgs) {
  try {
    const provider = smartContract.getChainProvider(chainId, {});
    const compilerMetadata = await smartContract.fetchContractMetadataFromAddress(contractAddress, provider, storage);
    const compilerVersion = compilerMetadata.metadata.compiler.version;
    const sources = await smartContract.fetchSourceFilesFromMetadata(compilerMetadata, storage);
...
async function isVerifiedOnEtherscan(contractAddress, chainId, explorerAPIUrl, explorerAPIKey, clientId) {
  ...
  invariant__default["default"](await smartContract.isContractDeployed(contractAddress, provider), "Contract not deployed yet.");
...
async function fetchConstructorParams(explorerAPIUrl, explorerAPIKey, contractAddress, abi, provider, storage) {
  const constructorParamTypes = smartContract.extractConstructorParamsFromAbi(abi);
...
async function fetchDeployBytecodeFromPublishedContractMetadata(contractAddress, provider, storage) {
  const compilerMetaUri = await smartContract.resolveContractUriFromAddress(contractAddress, provider);
  if (compilerMetaUri) {
    const contract = new ethers.Contract(smartContract.getContractPublisherAddress(), ContractPublisherAbi__default["default"], smartContract.getChainProvider("polygon", {}));
    ...
    const pubmeta = await Promise.all(publishedMetadataUri.filter(uri => uri.length > 0).map(uri => smartContract.fetchExtendedReleaseMetadata(uri, storage)));
 ...
 * @public
 */
class ContractVerifier extends smartContract.RPCConnectionHandler {
  constructor(network, options, storage) {
...
function extractFunctionParamsFromAbi(abi, functionName) {
  const parsedAbi = smartContract.AbiSchema.parse(abi || []);
...
async function computeCloneFactoryAddress(provider, storage, create2Factory, clientId, secretKey) {
  if (!create2Factory) {
    create2Factory = await smartContract.getCreate2FactoryAddress(provider);
  }
  return (await smartContract.computeDeploymentInfo("infra", provider, storage, create2Factory, {
...
async function deployCreate2Factory(signer, options) {
  ...
  const commonFactoryExists = await smartContract.isContractDeployed(smartContract.COMMON_FACTORY, signer.provider);
  if (commonFactoryExists) {
    return smartContract.COMMON_FACTORY;
  }
  const enforceEip155 = await smartContract.isEIP155Enforced(signer.provider);
  ...
  const deploymentInfo = smartContract.CUSTOM_GAS_FOR_CHAIN[networkId] ? smartContract.getCreate2FactoryDeploymentInfo(chainId, {
    gasPrice: smartContract.CUSTOM_GAS_FOR_CHAIN[networkId].gasPrice,
    gasLimit: smartContract.CUSTOM_GAS_FOR_CHAIN[networkId].gasLimit
  }) : smartContract.getCreate2FactoryDeploymentInfo(chainId, {});
  const factoryExists = await smartContract.isContractDeployed(deploymentInfo.deployment, signer.provider);

  // deploy community factory if not already deployed
  if (!factoryExists) {
    const gasPrice = smartContract.CUSTOM_GAS_FOR_CHAIN[networkId]?.gasPrice ? smartContract.CUSTOM_GAS_FOR_CHAIN[networkId].gasPrice : 100 * 10 ** 9;
    const gasLimit = smartContract.CUSTOM_GAS_FOR_CHAIN[networkId]?.gasLimit ? smartContract.CUSTOM_GAS_FOR_CHAIN[networkId].gasLimit : 100000;
 ...
 * @public
 */
const SUPPORTED_CHAIN_IDS = [smartContract.ChainId.Mainnet, smartContract.ChainId.Goerli, smartContract.ChainId.Polygon, smartContract.ChainId.Mumbai, smartContract.ChainId.Fantom, smartContract.ChainId.FantomTestnet, smartContract.ChainId.Avalanche, smartContract.ChainId.AvalancheFujiTestnet, smartContract.ChainId.Optimism, smartContract.ChainId.OptimismGoerli, smartContract.ChainId.Arbitrum, smartContract.ChainId.ArbitrumGoerli, smartContract.ChainId.BinanceSmartChainMainnet, smartContract.ChainId.BinanceSmartChainTestnet, smartContract.ChainId.Hardhat, smartContract.ChainId.Localhost];
...
function getDefaultTrustedForwarders(chainId) {
  ...
  const biconomyForwarder = chainEnum ? smartContract.CONTRACT_ADDRESSES[chainEnum].biconomyForwarder : smartContract.AddressZero;
  const openzeppelinForwarder = chainEnum ? smartContract.CONTRACT_ADDRESSES[chainEnum].openzeppelinForwarder : smartContract.AddressZero;
  return [openzeppelinForwarder, biconomyForwarder].filter(a => a !== smartContract.AddressZero);
}
...
async function getDeployArguments(contractType, metadata, contractURI, signer, storage) {
  ...
  if (!chainEnum) {
    const forwarder = await smartContract.computeForwarderAddress(signer.provider, storage);
    trustedForwarders = [forwarder];
  } else {
    trustedForwarders = contractType === smartContract.PackInitializer.contractType ? [] : getDefaultTrustedForwarders(chainId);
  }
  ...
  switch (contractType) {
    case smartContract.NFTDropInitializer.contractType:
    case smartContract.NFTCollectionInitializer.contractType:
      const erc721metadata = await smartContract.NFTDropInitializer.schema.deploy.parseAsync(metadata);
    ...
    case smartContract.SignatureDropInitializer.contractType:
      const signatureDropmetadata = await smartContract.SignatureDropInitializer.schema.deploy.parseAsync(metadata);
    ...
    case smartContract.MultiwrapInitializer.contractType:
      const multiwrapMetadata = await smartContract.MultiwrapInitializer.schema.deploy.parseAsync(metadata);
    ...
    case smartContract.EditionDropInitializer.contractType:
    case smartContract.EditionInitializer.contractType:
      const erc1155metadata = await smartContract.EditionDropInitializer.schema.deploy.parseAsync(metadata);
    ...
    case smartContract.TokenDropInitializer.contractType:
    case smartContract.TokenInitializer.contractType:
      const erc20metadata = await smartContract.TokenInitializer.schema.deploy.parseAsync(metadata);
    ...
    case smartContract.VoteInitializer.contractType:
      const voteMetadata = await smartContract.VoteInitializer.schema.deploy.parseAsync(metadata);
    ...
    case smartContract.SplitInitializer.contractType:
      const splitsMetadata = await smartContract.SplitInitializer.schema.deploy.parseAsync(metadata);
    ...
    case smartContract.MarketplaceInitializer.contractType:
    case smartContract.MarketplaceV3Initializer.contractType:
      const marketplaceMetadata = await smartContract.MarketplaceInitializer.schema.deploy.parseAsync(metadata);
    ...
    case smartContract.PackInitializer.contractType:
      const packsMetadata = await smartContract.PackInitializer.schema.deploy.parseAsync(metadata);
...
async function getTrustedForwarders(provider, storage, contractName) {
  ...
  const chainEnum = SUPPORTED_CHAIN_IDS.find(c => c === chainId);
  const trustedForwarders = contractName && contractName === smartContract.PackInitializer.name ? [] : chainEnum ? getDefaultTrustedForwarders(chainId) : [await smartContract.computeForwarderAddress(provider, storage)]; // TODO: make this default for all chains (standard + others)
...
const APPROVED_IMPLEMENTATIONS = {
  ...
  [smartContract.ChainId.Avalanche]: {
  ...
  [smartContract.ChainId.Optimism]: {
  ...
  [smartContract.ChainId.Arbitrum]: {
  ...
  [smartContract.ChainId.BinanceSmartChainMainnet]: {
  ...
  [smartContract.ChainId.Mumbai]: {
    "nft-drop": "0xC4903c1Ff5367b9ac2c349B63DC2409421AaEE2a",
    "edition-drop": "0xCcddcec1831646Beff2753249f1B9C580327E89F",
    "token-drop": "0x1b5947e1a2d5a29D0df20931DeAB0B87818209B9",
    "signature-drop": "0x2dF9851af45dd41C8584ac55D983C604da985Bc7"
  },
  [smartContract.ChainId.FantomTestnet]: {
  ...
  [smartContract.ChainId.AvalancheFujiTestnet]: {
    "nft-drop": "0xD11c97DD5F5546B5bBd630D7D1d7327481B0b92C",
  ...
  [smartContract.ChainId.OptimismGoerli]: {
  ...
  [smartContract.ChainId.ArbitrumGoerli]: {
  ...
  [smartContract.ChainId.BinanceSmartChainTestnet]: {
    "nft-drop": "",
    "edition-drop": "",
    "token-drop": "",
    "signature-drop": "" // TODO
 ...
 * @internal
 */
class ContractFactory extends smartContract.ContractWrapper {
  // Map from contract type to version to deploy specific versions by default
  DEFAULT_VERSION_MAP = {
    [smartContract.NFTDropInitializer.contractType]: 3,
    [smartContract.NFTCollectionInitializer.contractType]: 1,
    [smartContract.SignatureDropInitializer.contractType]: 4,
    [smartContract.MultiwrapInitializer.contractType]: 1,
    [smartContract.EditionDropInitializer.contractType]: 2,
    [smartContract.EditionInitializer.contractType]: 1,
    [smartContract.TokenDropInitializer.contractType]: 2,
    [smartContract.TokenInitializer.contractType]: 1,
    [smartContract.VoteInitializer.contractType]: 1,
    [smartContract.SplitInitializer.contractType]: 1,
    [smartContract.MarketplaceInitializer.contractType]: 2,
    [smartContract.MarketplaceV3Initializer.contractType]: 3,
    [smartContract.PackInitializer.contractType]: 2
  };
  constructor(factoryAddr, network, storage, options) {
    super(network, factoryAddr, TWFactoryAbi__default["default"], options, storage);
    this.storage = storage;
  }
  deploy = /* @__PURE__ */smartContract.buildTransactionFunction(async (contractType, contractMetadata, eventEmitter, version, options, onExecute) => {
    const contract = smartContract.PREBUILT_CONTRACTS_MAP[contractType];
  ...
  deployProxyByImplementation = /* @__PURE__ */smartContract.buildTransactionFunction(async (implementationAddress, implementationAbi, initializerFunction, initializerArgs, eventEmitter, saltForProxyDeploy, onExecute) => {
  ...
  async getDeployArguments(contractType, metadata, contractURI) {
    let trustedForwarders = contractType === smartContract.PackInitializer.contractType ? [] : await this.getDefaultTrustedForwarders();
    ...
    switch (contractType) {
      ...
      case smartContract.MarketplaceV3Initializer.contractType:
        const marketplaceV3Metadata = await smartContract.MarketplaceV3Initializer.schema.deploy.parseAsync(metadata);
  ...
  async getLatestVersion(contractType) {
    const name = smartContract.getContractName(contractType);
    if (!name) {
 ...
 * @internal
 */
class ContractRegistry extends smartContract.ContractWrapper {
  ...
  async getContractAddresses(walletAddress) {
    ...
    return (await this.read("getAll", [await smartContract.resolveAddress(walletAddress)])).filter(adr => ethers.utils.isAddress(adr) && adr.toLowerCase() !== ethers.constants.AddressZero);
  }
  addContract = /* @__PURE__ */smartContract.buildTransactionFunction(async contractAddress => {
    return await this.addContracts.prepare([contractAddress]);
  });
  addContracts = /* @__PURE__ */smartContract.buildTransactionFunction(async contractAddresses => {
    const deployerAddress = await this.getSignerAddress();
    const contractEncoder = new smartContract.ContractEncoder(this);
    const encoded = (await Promise.all(contractAddresses.map(addr => smartContract.resolveAddress(addr)))).map(address => contractEncoder.encode("add", [deployerAddress, address]));
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this,
      method: "multicall",
      args: [encoded]
  ...
  removeContract = /* @__PURE__ */smartContract.buildTransactionFunction(async contractAddress => {
    return await this.removeContracts.prepare([contractAddress]);
  });
  removeContracts = /* @__PURE__ */smartContract.buildTransactionFunction(async contractAddresses => {
    const deployerAddress = await this.getSignerAddress();
    const contractEncoder = new smartContract.ContractEncoder(this);
    const encoded = (await Promise.all(contractAddresses.map(addr => smartContract.resolveAddress(addr)))).map(address => contractEncoder.encode("remove", [deployerAddress, address]));
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this,
      method: "multicall",
      args: [encoded]
...
const LoyaltyCardContractInput = /* @__PURE__ */(() => smartContract.CommonContractSchema.merge(smartContract.CommonRoyaltySchema).merge(smartContract.CommonSymbolSchema))();
const LoyaltyCardContractDeploy = /* @__PURE__ */(() => LoyaltyCardContractInput.merge(smartContract.CommonPlatformFeeSchema).merge(smartContract.CommonPrimarySaleSchema).merge(smartContract.CommonTrustedForwarderSchema))();

const AirdropContractInput = /* @__PURE__ */(() => smartContract.CommonContractSchema.merge(smartContract.CommonSymbolSchema))();
const AirdropContractDeploy = /* @__PURE__ */(() => AirdropContractInput.merge(smartContract.CommonTrustedForwarderSchema))();
...
async function directDeployDeterministic(bytecode, abi, signer, constructorArgs, saltForCreate2) {
  ...
  const constructorParams = smartContract.extractConstructorParamsFromAbi(abi);
  ...
  const address = smartContract.computeDeploymentAddress(bytecodePrefixed, encodedArgs, create2Factory, saltForCreate2);
  const contractDeployed = await smartContract.isContractDeployed(address, signer.provider);
  let initBytecodeWithSalt = "";
  if (!contractDeployed) {
    console.debug(`deploying contract via create2 factory at: ${address}`);
    initBytecodeWithSalt = smartContract.getInitBytecodeWithSalt(bytecodePrefixed, encodedArgs, saltForCreate2);
    const tx = {
      to: create2Factory,
...
async function directDeployDeterministicWithUri(publishMetadataUri, signer, storage, constructorArgs, saltForCreate2) {
  ...
  const {
    compilerMetadata,
    extendedMetadata
  } = await smartContract.fetchAndCacheDeployMetadata(publishMetadataUri, storage);
...
async function directDeployDeterministicPublished(contractName, publisherAddress) {
  ...
  const publishMetadataUri = (await smartContract.fetchPublishedContractFromPolygon(publisherAddress, contractName, contractVersion, storage, clientId, secretKey)).metadataUri;
  const {
    compilerMetadata,
    extendedMetadata
  } = await smartContract.fetchAndCacheDeployMetadata(publishMetadataUri, storage);
 ...
 * @public
 */
class ThirdwebSDK extends smartContract.RPCConnectionHandler {
  /**
  ...
  static fromSigner(signer, network) {
    ...
    if (network) {
      try {
        const provider = smartContract.getChainProvider(network, options);
  ...
  constructor(network) {
    ...
    super(network, options);
    smartContract.setSupportedChains(options?.supportedChains);
    ...
    this._publisher = new smartContract.ContractPublisher(network, this.options, this.storageHandler);
  }
  get auth() {
  ...
  async getContract(address, contractTypeOrABI) {
    const resolvedAddress = await smartContract.resolveAddress(address);
    ...
    if (!contractTypeOrABI || contractTypeOrABI === "custom") {
      try {
        ...
        if (resolvedContractType && resolvedContractType !== "custom") {
          ...
          const contractAbi = await smartContract.PREBUILT_CONTRACTS_MAP[resolvedContractType].getAbi(resolvedAddress, this.getProvider(), this.storage);
    ...
    else if (typeof contractTypeOrABI === "string" && contractTypeOrABI in smartContract.PREBUILT_CONTRACTS_MAP) {
      newContract = await smartContract.PREBUILT_CONTRACTS_MAP[contractTypeOrABI].initialize(this.getSignerOrProvider(), resolvedAddress, this.storage, this.options);
    }
  ...
  async resolveContractType(contractAddress) {
    try {
      const contract = new ethers.Contract(await smartContract.resolveAddress(contractAddress), IThirdwebContractABI__default["default"],
      ...
      .replace(/\x00/g, "");
      return smartContract.getContractTypeForRemoteName(remoteContractType);
    } catch (err) {
      return "custom";
    }
  }
  ...
  async getContractList(walletAddress) {
    ...
    const addresses = (await (await this.deployer.getRegistry())?.getContractAddresses(await smartContract.resolveAddress(walletAddress))) || [];
    const chainId = (await this.getProvider().getNetwork()).chainId;
  ...
  async getContractFromAbi(address, abi) {
    ...
    const [, provider] = smartContract.getSignerAndProvider(this.getSignerOrProvider(), this.options);
    const parsedABI = typeof abi === "string" ? JSON.parse(abi) : abi;
    // TODO we still might want to lazy-fy this
    const contract = new smartContract.SmartContract(this.getSignerOrProvider(), resolvedAddress, await smartContract.getCompositeABI(resolvedAddress, smartContract.AbiSchema.parse(parsedABI), provider, this.options, this.storage), this.storageHandler, this.options, (await provider.getNetwork()).chainId);
  ...
  async getBalance(address) {
    return smartContract.fetchCurrencyValue(this.getProvider(), smartContract.NATIVE_TOKEN_ADDRESS, await this.getProvider().getBalance(await smartContract.resolveAddress(address)));
  }
...
function addChainToSupportedChains(network, options) {
  if (smartContract.isChainConfig(network)) {
    options = {
      ...options,
 ...
 * @public
 */
class ContractDeployer extends smartContract.RPCConnectionHandler {
  /**
   * @internal
   ...
   */
  deployNFTCollection = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    return await this.deployBuiltInContract.prepare(smartContract.NFTCollectionInitializer.contractType, metadata, "latest", options);
  });
   ...
   * @returns the address of the deployed contract
   */
  deployNFTDrop = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    return await this.deployBuiltInContract.prepare(smartContract.NFTDropInitializer.contractType, metadata, "latest", options);
  });
   ...
   * @returns the address of the deployed contract
   */
  deployLoyaltyCard = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
   ...
   * @returns the address of the deployed contract
   */
  deployOpenEdition = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    const parsedMetadata = await smartContract.DropErc721ContractSchema.deploy.parseAsync(metadata);
    const contractURI = await this.storage.upload(parsedMetadata);
    const chainId = (await this.getProvider().getNetwork()).chainId;
    const trustedForwarders = getDefaultTrustedForwarders(chainId);
    // add default forwarders to any custom forwarders passed in
   ...
   * @returns the address of the deployed contract
   */
  deploySignatureDrop = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    return await this.deployBuiltInContract.prepare(smartContract.SignatureDropInitializer.contractType, metadata, "latest", options);
  });
   ...
   * @beta
   */
  deployMultiwrap = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    return await this.deployBuiltInContract.prepare(smartContract.MultiwrapInitializer.contractType, metadata, "latest", options);
  });
   ...
   * @returns the address of the deployed contract
   */
  deployEdition = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    return await this.deployBuiltInContract.prepare(smartContract.EditionInitializer.contractType, metadata, "latest", options);
  });
   ...
   * @returns the address of the deployed contract
   */
  deployEditionDrop = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    return await this.deployBuiltInContract.prepare(smartContract.EditionDropInitializer.contractType, metadata, "latest", options);
  });
   ...
   * @returns the address of the deployed contract
   */
  deployToken = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    return await this.deployBuiltInContract.prepare(smartContract.TokenInitializer.contractType, metadata, "latest", options);
  });
   ...
   * @returns the address of the deployed contract
   */
  deployTokenDrop = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    return await this.deployBuiltInContract.prepare(smartContract.TokenDropInitializer.contractType, metadata, "latest", options);
  });
   ...
   * @returns the address of the deployed contract
   */
  deployMarketplace = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    return await this.deployBuiltInContract.prepare(smartContract.MarketplaceInitializer.contractType, metadata, "latest", options);
  });
   ...
   * @returns the address of the deployed contract
   */
  deployMarketplaceV3 = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    return await this.deployBuiltInContract.prepare(smartContract.MarketplaceV3Initializer.contractType, metadata, "latest", options);
  });
   ...
   * @returns the address of the deployed contract
   */
  deployPack = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    return await this.deployBuiltInContract.prepare(smartContract.PackInitializer.contractType, metadata, "latest", options);
  });
   ...
   * @returns the address of the deployed contract
   */
  deploySplit = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    return await this.deployBuiltInContract.prepare(smartContract.SplitInitializer.contractType, metadata, "latest", options);
  });
   ...
   * @returns the address of the deployed contract
   */
  deployVote = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    return await this.deployBuiltInContract.prepare(smartContract.VoteInitializer.contractType, metadata, "latest", options);
  });
  deployAirdropERC20 = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
  ...
  deployAirdropERC721 = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    const parsedMetadata = await AirdropContractDeploy.parseAsync(metadata);
    const contractURI = await this.storage.upload(parsedMetadata);
    const chainId = (await this.getProvider().getNetwork()).chainId;
    const trustedForwarders = getDefaultTrustedForwarders(chainId);
  ...
  deployAirdropERC1155 = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (metadata, options) => {
    const parsedMetadata = await AirdropContractDeploy.parseAsync(metadata);
    const contractURI = await this.storage.upload(parsedMetadata);
    const chainId = (await this.getProvider().getNetwork()).chainId;
    const trustedForwarders = getDefaultTrustedForwarders(chainId);
   ...
   */
  deployBuiltInContract = /* @__PURE__ */smartContract.buildDeployTransactionFunction((() => {
    var _this = this;
    return async function (contractType, contractMetadata) {
      ...
      const parsedMetadata = {
        app_uri: smartContract.PREBUILT_CONTRACTS_APPURI_MAP[contractType],
        ...(await smartContract.PREBUILT_CONTRACTS_MAP[contractType].schema.deploy.parseAsync(contractMetadata))
      };
      if (_this.hasLocalFactory()) {
        ...
        factory.on(smartContract.EventType.Transaction, _this.transactionListener);
        return factory.deploy.prepare(contractType, parsedMetadata, _this.events, parsedVersion, options, () => {
          factory.off(smartContract.EventType.Transaction, _this.transactionListener);
        });
      }

      // For all other chains, fetch from published contracts
      // resolve contract name from type
      const contractName = smartContract.getContractName(contractType);
   ...
   */
  deployReleasedContract = /* @__PURE__ */smartContract.buildDeployTransactionFunction((() => {
    var _this2 = this;
   ...
   */
  deployViaFactory = /* @__PURE__ */smartContract.buildTransactionFunction(async (factoryAddress, implementationAddress, implementationAbi, initializerFunction, initializerArgs, saltForProxyDeploy) => {
    const resolvedFactoryAddress = await smartContract.resolveAddress(factoryAddress);
    const resolvedImplementationAddress = await smartContract.resolveAddress(implementationAddress);
    const signer = this.getSigner();
    invariant__default["default"](signer, "signer is required");
    ...
    proxyFactory.on(smartContract.EventType.Transaction, this.transactionListener);
    return await proxyFactory.deployProxyByImplementation.prepare(resolvedImplementationAddress, implementationAbi, initializerFunction, initializerArgs, this.events, saltForProxyDeploy, () => {
      proxyFactory.off(smartContract.EventType.Transaction, this.transactionListener);
    });
  });
   ...
   * @param initializerArgs
   */
  deployProxy = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (implementationAddress, implementationAbi, initializerFunction, initializerArgs) => {
    const resolvedAddress = await smartContract.resolveAddress(implementationAddress);
   ...
   * @param options
   */
  deployViaAutoFactory = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (publishMetadataUri, deployMetadata, signer, initializerFunction, paramValues, options) => {
    ...
    const deploymentInfo = await smartContract.getDeploymentInfo(publishMetadataUri, this.storage, this.getProvider(), create2Factory, this.options.clientId, this.options.secretKey);
    ...
    await smartContract.deployWithThrowawayDeployer(signer, transactionsForThrowawayDeployer, options);

    // send each transaction directly to Create2 factory
    // process txns one at a time
    for (const tx of transactionsforDirectDeploy) {
      try {
        await smartContract.deployContractDeterministic(signer, tx, options);
      } catch (e) {
   ...
   * @param chainId
   */
  deployViaCustomFactory = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (constructorParamValues, deployMetadata, signer, chainId) => {
    ...
    const resolvedCustomFactoryAddress = await smartContract.resolveAddress(customFactoryAddress);
    ...
    const customFactoryMetadata = await smartContract.fetchContractMetadataFromAddress(resolvedCustomFactoryAddress, this.getProvider(), this.storage, this.options);
    ...
    const deployTransaction = await smartContract.Transaction.fromContractInfo({
  ...
  async getRegistry() {
    ...
    return this._registry = this.getProvider().getNetwork().then(async _ref2 => {
      ...
      const registryAddress = smartContract.getContractAddressByChainId(chainId, "twRegistry");
      if (!registryAddress) {
        return undefined;
      }
  ...
  async getFactory() {
    ...
    return this._factory = this.getProvider().getNetwork().then(async _ref3 => {
      ...
      const factoryAddress = smartContract.getContractAddressByChainId(chainId, "twFactory");
      if (!factoryAddress) {
        return undefined;
      }
   ...
   * @param options
   */
  deployContractFromUri = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (publishMetadataUri, constructorParamValues, options) => {
    const signer = this.getSigner();
    invariant__default["default"](signer, "A signer is required");
    const {
      compilerMetadata,
      extendedMetadata
    } = await smartContract.fetchAndCacheDeployMetadata(publishMetadataUri, this.storage);
    ...
    const constructorParamTypes = smartContract.extractConstructorParamsFromAbi(compilerMetadata.abi).map(p => p.type);
   ...
   */
  deployContractWithAbi = /* @__PURE__ */smartContract.buildDeployTransactionFunction(async (abi, bytecode, constructorParams) => {
    const signer = this.getSigner();
    const provider = this.getProvider();
    ...
    return new smartContract.DeployTransaction({
  ...
  async getTransactionsForDeploy(publishMetadataUri, options) {
    ...
    if (extendedMetadata && extendedMetadata.factoryDeploymentData && (extendedMetadata.isDeployableViaProxy || extendedMetadata.isDeployableViaFactory || extendedMetadata.deployType === "autoFactory") && !forceDirectDeploy) {
      ...
      const implementationAddress = extendedMetadata.factoryDeploymentData.implementationAddresses[chainId];
      if (!implementationAddress || extendedMetadata.deployType === "autoFactory") {
        const create2FactoryAddress = await smartContract.getCreate2FactoryAddress(this.getProvider());
        transactions.push({
          contractType: "create2Factory",
          addresses: [create2FactoryAddress]
        });
        const deploymentInfo = await smartContract.getDeploymentInfo(publishMetadataUri, this.storage, this.getProvider(), create2FactoryAddress, this.options.clientId, this.options.secretKey);
        const transactionsToSend = deploymentInfo.filter(i => i.transaction.data && i.transaction.data.length > 0);
        const transactionsforDirectDeploy = transactionsToSend.filter(i => {
          return i.type !== "infra";
        }).map(i => i.transaction);
        ...
        const transactionBatches = smartContract.createTransactionBatches(transactionsForThrowawayDeployer);
      ...
      transactions = (await Promise.all(transactions.map(async tx => {
        const addresses = (await Promise.all(tx.addresses.map(async address => {
          const isDeployed = await smartContract.isContractDeployed(address, provider);
  ...
  async fetchPublishedContractFromPolygon(publisherAddress, contractName, version) {
    const address = await smartContract.resolveAddress(publisherAddress);
  ...
  hasLocalFactory() {
    return !!smartContract.getProcessEnv("factoryAddress");
  }
}
