...
class Multiwrap extends erc721Standard.StandardErc721 {
  static contractRoles = smartContract.MULTIWRAP_CONTRACT_ROLES;
  ...
  constructor(network, address, storage) {
    ...
    this.metadata = new smartContract.ContractMetadata(this.contractWrapper, smartContract.MultiwrapContractSchema, this.storage);
    this.app = new smartContract.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new smartContract.ContractRoles(this.contractWrapper, Multiwrap.contractRoles);
  ...
  async getWrappedContents(wrappedTokenId) {
    ...
    for (const token of wrappedTokens) {
      switch (token.tokenType) {
        case 0:
          {
            const tokenMetadata = await smartContract.fetchCurrencyMetadata(this.contractWrapper.getProvider(), token.assetContract);
            erc20Tokens.push({
   ...
   */
  wrap = /* @__PURE__ */smartContract.buildTransactionFunction(async (contents, wrappedTokenMetadata, recipientAddress) => {
    const [uri, tokens, recipient] = await Promise.all([smartContract.uploadOrExtractURI(wrappedTokenMetadata, this.storage), this.toTokenStructList(contents), smartContract.resolveAddress(recipientAddress ? recipientAddress : await this.contractWrapper.getSignerAddress())]);
   ...
   */
  unwrap = /* @__PURE__ */smartContract.buildTransactionFunction(async (wrappedTokenId, recipientAddress) => {
    const recipient = await smartContract.resolveAddress(recipientAddress ? recipientAddress : await this.contractWrapper.getSignerAddress());
    return smartContract.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "unwrap",
  ...
  async toTokenStructList(contents) {
    ...
    if (contents.erc20Tokens) {
      for (const erc20 of contents.erc20Tokens) {
        const normalizedQuantity = await smartContract.normalizePriceValue(provider, erc20.quantity, erc20.contractAddress);
