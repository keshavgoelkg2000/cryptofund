...
class Vote {
  ...
  constructor(network, address, storage) {
    ...
    this.metadata = new smartContract.ContractMetadata(this.contractWrapper, smartContract.VoteContractSchema, this.storage);
  ...
  async hasVoted(proposalId, account) {
    ...
    return this.contractWrapper.read("hasVoted", [proposalId, await smartContract.resolveAddress(account)]);
  }
  ...
  async balanceOfToken(tokenAddress) {
    const erc20 = new ethers.Contract(await smartContract.resolveAddress(tokenAddress), ERC20Abi__default["default"], this.contractWrapper.getProvider());
    return await smartContract.fetchCurrencyValue(this.contractWrapper.getProvider(), tokenAddress, await erc20.balanceOf(this.contractWrapper.address));
  }
  ...
  async settings() {
    ...
    const votingTokenMetadata = await smartContract.fetchCurrencyMetadata(this.contractWrapper.getProvider(), votingTokenAddress);
    return {
   ...
   */
  propose = /* @__PURE__ */smartContract.buildTransactionFunction(async (description, executions) => {
    if (!executions) {
      executions = [{
   ...
   */
  vote = /* @__PURE__ */smartContract.buildTransactionFunction((() => {
    var _this = this;
   ...
   */
  execute = /* @__PURE__ */smartContract.buildTransactionFunction(async proposalId => {
    await this.ensureExists(proposalId);
    const proposal = await this.get(proposalId);
    const tos = proposal.executions.map(p => p.toAddress);
    const values = proposal.executions.map(p => p.nativeTokenValue);
