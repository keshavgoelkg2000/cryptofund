...
function matchesPrebuiltAbi(contractWrapper, abi) {
  return smartContract.hasMatchingAbi(smartContract.AbiSchema.parse(contractWrapper.abi || []), [abi]);
}
...
function toUnits(amount, decimals) {
  return ethers.utils.parseUnits(smartContract.AmountSchema.parse(amount), decimals);
}
...
async function deployContractDeterministicRaw(signer, bytecode, encodedArgs, create2FactoryAddress, options, predictedAddress) {
  ...
  const contractDeployed = predictedAddress ? await smartContract.isContractDeployed(predictedAddress, signer.provider) : false;
  if (!contractDeployed) {
    console.debug(`deploying contract via create2 factory at: ${predictedAddress}`);
    const initBytecodeWithSalt = smartContract.getInitBytecodeWithSalt(bytecode, encodedArgs);
    const tx = {
...
const PropertiesInput = /* @__PURE__ */(() => zod.z.object({}).catchall(zod.z.union([smartContract.BigNumberTransformSchema, zod.z.unknown()])))();
...
const EditionMetadataOutputSchema = /* @__PURE__ */(() => zod.z.object({
  supply: smartContract.BigNumberSchema,
  metadata: smartContract.CommonNFTOutput
}))();
...
const EditionMetadataWithOwnerOutputSchema = /* @__PURE__ */(() => EditionMetadataOutputSchema.extend({
  owner: zod.z.string(),
  quantityOwned: smartContract.BigNumberSchema
}))();
...
const EditionMetadataInputSchema = /* @__PURE__ */(() => zod.z.object({
  supply: smartContract.BigNumberishSchema,
  metadata: smartContract.CommonNFTInput
}))();
...
const EditionMetadataInputOrUriSchema = /* @__PURE__ */(() => zod.z.object({
  supply: smartContract.BigNumberishSchema,
  metadata: smartContract.NFTInputOrUriSchema
}))();
...
const TokenMintInputSchema = /* @__PURE__ */(() => zod.z.object({
  toAddress: smartContract.AddressOrEnsSchema,
  amount: smartContract.AmountSchema
}))();
